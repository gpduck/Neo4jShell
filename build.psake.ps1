...
Task CopyLibraries {
  ...
  $TargetBin | ForEach-Object {
    if(-not (Test-Path $_)) {
      $null = mkdir $_ -Force
    }
  }
  
  copy "$baseDir\vendor\packages\Neo4j.Driver.*\lib\dotnet\*.*" $TargetBin
  copy "$baseDir\vendor\packages\rda.SocketsForPCL.*\lib\net45\*.*" $TargetBin
}
